...
var index = (function (dir) {
    if (dir === void 0) { dir = process.cwd(); }
    return __awaiter$1(void 0, void 0, Promise, function () {
        function findFiles() {
            return __awaiter$1(this, void 0, void 0, function () {
                var _this = this;
                return __generator$1(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, forEach(unfinishedFinds, function (find, index) { return __awaiter$1(_this, void 0, void 0, function () {
                                var children;
                                return __generator$1(this, function (_a) {
                                    switch (_a.label) {
                                        case 0: return [4 /*yield*/, readdirAsync(find)];
                                        case 1:
                                            children = _a.sent();
                        ...
                        case 1:
                            _a.sent();
                            if (!(unfinishedFinds.length !== 0)) return [3 /*break*/, 3];
                            return [4 /*yield*/, findFiles()];
                        case 2:
                            _a.sent();
                            _a.label = 3;
        ...
        var unfinishedFinds, finds, firstDepthFiles;
        return __generator$1(this, function (_a) {
            switch (_a.label) {
                case 0:
                    finds = [];
                    return [4 /*yield*/, readdirAsync(dir)];
                case 1:
                    firstDepthFiles = _a.sent();
                    if (firstDepthFiles)
                        unfinishedFinds = firstDepthFiles.map(function (val) { return dir + "/" + val; });
                    else
                        return [2 /*return*/, []];
                    return [4 /*yield*/, findFiles()];
                case 2:
                    _a.sent();
                    return [2 /*return*/, finds.map(function (find) { return find.replace(dir + "/", ''); })];
            }
        });
    });
});
...
const strip = (file, options) => {
  if (!file) return file;
  if (isObject$1(file) && file.path) {
    return strip.file(file, options);
  }

  let filepath = strip.increment(file, options);
  let extname = path__default.extname(filepath);
  let dirname = strip.increment(path__default.dirname(filepath), options);
  let stem = strip.increment(path__default.basename(filepath, extname), options);
  return path__default.join(dirname, stem + extname);
};
...
strip.increment = (input, options = {}) => {
  if (typeof input === 'string' && input !== '') {
    let suffix = options.removeRawNumbers === true ? constants$2.REGEX_RAW_NUMBERS : '';
    let source = constants$2.REGEX_SOURCE + suffix;
...
strip.dirname = (filepath, options) => {
  return strip.increment(path__default.dirname(filepath), options);
};
...
strip.stem = (filepath, options) => {
  return strip.increment(path__default.basename(filepath, path__default.extname(filepath)), options);
};
...
strip.basename = (filepath, options) => {
  let extname = path__default.extname(filepath);
  let stem = path__default.basename(filepath, extname);
...
strip.file = (file, options = {}) => {
  if (!isObject$1(file)) return file;
  ...
  if (file.dir === void 0) file.dir = path__default.dirname(file.path);
  if (file.ext === void 0) file.ext = path__default.extname(file.path);
  if (file.base === void 0) file.base = path__default.basename(file.path);
  if (file.name === void 0) file.name = path__default.basename(file.path, file.ext);

  file.name = strip.increment(file.name, options);
  file.dir = strip.increment(file.dir, options);
  file.base = file.name + file.ext;

  file.path = path__default.join(file.dir, file.base);
  return file;
};

var stripFilenameIncrement = strip;
...
increment.path = (filepath, options = {}) => {
  return path__default.format(increment.file(path__default.parse(filepath), options));
};
...
increment.file = (file, options = {}) => {
  ...
  if (file.path && Object.keys(file).length === 1) {
    let temp = file.path;
    file = path__default.parse(file.path);
    file.path = temp;
  }
  ...
  if (options.strip === true) {
    file.name = stripFilenameIncrement.increment(file.name, options);
    file.dir = stripFilenameIncrement.increment(file.dir, options);
    file.base = file.name + file.ext;
  }

  if (options.fs === true) {
    let name = file.name;
    let dest = path__default.format(file);

    while (fs__default.existsSync(dest)) {
      file.base = fn(name, start++) + file.ext;
      dest = path__default.format(file);
    }
  } else {
...
var addFilenameIncrement = increment;
...
const write = (filepath, data, options, callback) => {
  ...
  const promise = mkdir(path__default.dirname(destpath), { recursive: true, ...options })
    .then(() => {
      return new Promise((resolve, reject) => {
        fs__default.createWriteStream(destpath, opts)
...
write.sync = (filepath, data, options) => {
  ...
  mkdirSync(path__default.dirname(destpath), { recursive: true, ...options });
  fs__default.writeFileSync(destpath, ensureNewline(data, opts), opts);
...
write.stream = (filepath, options) => {
  ...
  mkdirSync(path__default.dirname(destpath), { recursive: true, ...options });
  return fs__default.createWriteStream(destpath, opts);
};
...
const incrementName = (destpath, options = {}) => {
  ...
  return addFilenameIncrement(destpath, options);
};
...
const exists = (filepath, destpath) => {
  return filepath === destpath && fs__default.existsSync(filepath);
};

const mkdir = (dirname, options) => {
  return new Promise(resolve => fs__default.mkdir(dirname, options, () => resolve()));
};

const mkdirSync = (dirname, options) => {
  try {
    fs__default.mkdirSync(dirname, options);
...
var watchFiles = [];
var generateCode = function (options) { return __awaiter(void 0, void 0, void 0, function () {
    var filter, allFiles, files, filesToGlob, imports, body;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                filter = createFilter(options.include, options.exclude);
                return [4 /*yield*/, index(process.cwd())];
            case 1:
                allFiles = _a.sent();
                files = allFiles.filter(function (file) {
                    return filter(path__default.resolve(file)) && file != options.file && file != "rollup.config.js";
                });
                filesToGlob = files.map(function (name) { return path__default.resolve(name); });
                imports = "";
                body = "";
                if (!options.justImport)
                    body += "export default [\n";
                watchFiles = [];
                filesToGlob.forEach(function (file, index) {
...
var index$1 = (function (options) {
    ...
    optionsArr.map(function (options) {
        if (!options.include)
            options.include = "./**";
        if (!options.exclude)
    ...
    return {
        ...
        resolveId: function (id) {
            return __awaiter(this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, asyncForEach(optionsArr, function (options) { return __awaiter(_this, void 0, void 0, function () {
                                var file;
                                return __generator(this, function (_a) {
                                    switch (_a.label) {
                                        case 0:
                                            if (!options)
                                                return [2 /*return*/, this.error("options must be passed.")];
                                            if (!options.file)
                                                return [2 /*return*/, this.error("options.file is required.")];
                                            if (id !== options.file)
                                                return [2 /*return*/, null];
                                            return [4 /*yield*/, read(id)];
                                        case 1:
                                            file = _a.sent();
                                            if (!(file === null)) return [3 /*break*/, 3];
                                            return [4 /*yield*/, write_1(id, "")];
                                        case 2:
                                            _a.sent();
                                            this.emitFile({
                                                id: id,
        ...
        transform: function (code, id) {
            return __awaiter(this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            options = optionsArr.find(function (opt) { return path__default.resolve(opt.file) === id; });
                            if (!options)
                                return [2 /*return*/, null];
                            return [4 /*yield*/, generateCode(options)];
                        case 1:
                            code = _a.sent();
                            watchFiles.forEach(function (file) { return _this.addWatchFile(file); });
